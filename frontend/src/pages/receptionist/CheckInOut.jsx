...
import { LogIn, LogOut, RefreshCcw, Loader2, AlertCircle, CheckCircle2, Users } from 'lucide-react';
import { getActiveVisits, checkInVisitor, checkOutVisitor } from '../../services/visitorService';
...
export default function CheckInOut() {
  ...
  const fetchVisits = useCallback(async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to load visits. Please try again.');
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <div className="mx-auto max-w-6xl">
      ...
      {!loading && visits.length === 0 && (
        <div className="flex flex-col items-center gap-3 rounded-xl border border-dashed border-slate-300 bg-white py-16 text-center">
          ...
          <p className="text-sm text-slate-400">Approved visitors will appear here for check-in.</p>
        </div>
      )}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
              <thead className="bg-slate-50">
                <tr>
                  <th scope="col" className="px-4 py-3 font-semibold text-slate-600">Visitor</th>
                  <th scope="col" className="px-4 py-3 font-semibold text-slate-600">Employee</th>
                  <th scope="col" className="px-4 py-3 font-semibold text-slate-600">Purpose</th>
                  <th scope="col" className="px-4 py-3 font-semibold text-slate-600">Visit Date</th>
                  <th scope="col" className="px-4 py-3 font-semibold text-slate-600">Expected Arrival</th>
                  <th scope="col" className="px-4 py-3 font-semibold text-slate-600">Status</th>
                  <th scope="col" className="px-4 py-3 text-right font-semibold text-slate-600">Action</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={visit._id} className="hover:bg-slate-50/70">
                      <td className="px-4 py-3">
                        <p className="font-medium text-slate-900">{visit.visitorId?.name || 'Unknown'}</p>
                        <p className="text-xs text-slate-500">{visit.visitorId?.phone || ''}</p>
                      </td>
                      <td className="px-4 py-3">
                        <p className="text-slate-700">{visit.employeeId?.name || 'Unknown'}</p>
                        <p className="text-xs text-slate-500">{visit.employeeId?.department || ''}</p>
                      </td>
                      <td className="px-4 py-3 text-slate-700">{visit.purpose}</td>
                      <td className="px-4 py-3 text-slate-700">{formatDate(visit.visitDate)}</td>
                      <td className="px-4 py-3 text-slate-700">{formatTime(visit.expectedArrivalTime)}</td>
                      <td className="px-4 py-3">
                        <span
                      ...
                      <td className="px-4 py-3 text-right">
                        {canCheckIn && (
                          ...
                          >
                            {isActing ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <LogIn className="h-3.5 w-3.5" />}
                            Check In
                          </button>
                        )}
                        {canCheckOut && (
                          ...
                          >
                            {isActing ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <LogOut className="h-3.5 w-3.5" />}
                            Check Out
                          </button>
                        )}
                        {isPending && (
                        ...
                        {!canCheckIn && !canCheckOut && !isPending && !isCheckedOut && (
